...
def get_crew_tanker():
    ...
    # Mengambil data crew berdasarkan FullName dan PhotoId
...
# Fungsi untuk membuat folder berdasarkan PhotoId dan menyalin foto
def create_folder_and_copy_photos(crew_tanker, source_base_dir, destination_base_dir):
    for crew in crew_tanker:
        # Mendapatkan nama crew dan PhotoId
        full_name = (crew.FullName).lower()
        photo_id = crew.PhotoId

        # Path folder baru yang akan dibuat berdasarkan PhotoId
        new_folder_path = os.path.join(destination_base_dir, photo_id)

        # Membuat folder jika belum ada
        os.makedirs(new_folder_path, exist_ok=True)

        # Path sumber foto berdasarkan struktur folder yang diberikan
        source_photo_path = os.path.join(source_base_dir, full_name.title(), "personaldata", "photo")
        ...
        if os.path.exists(source_photo_path):
            ...
            for file_name in os.listdir(source_photo_path):
                ...
                if os.path.isfile(source_file):
                    # Menyalin file foto ke dalam folder tujuan berdasarkan PhotoId
